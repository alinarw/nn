...
if __name__ == '__main__':
    ...
    w3 = init_param((o, h_nodes))
    b3 = init_param((o, ))

    pred = nn1(X, w1, b1, w2, b2, w3, b3)
    ...
    #Computing Cross-entropy criterion function + l2 regularisation
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=y, logits=pred)) + lam*(tf.nn.l2_loss(w1) + tf.nn.l2_loss(w2) + tf.nn.l2_loss(w3)) #lam*(tf.reduce_sum(tf.pow(w1, 2)) + tf.reduce_sum(tf.pow(w2, 2)) + tf.reduce_sum(tf.pow(w3, 2)))
